...
type SocketStats unix.TpacketStats

// GetPackets returns the number of packets seen by this socket.
func (s *SocketStats) GetPackets() uint {
	return uint(s.Packets)
}

// GetDrops returns the number of packets dropped on this socket.
func (s *SocketStats) GetDrops() uint {
	return uint(s.Drops)
}

// SocketStatsV3 is a struct where socket stats for TPacketV3 are stored
type SocketStatsV3 unix.TpacketStatsV3

// GetPackets returns the number of packets seen by this socket.
func (s *SocketStatsV3) GetPackets() uint {
	return uint(s.Packets)
}

// GetDrops returns the number of packets dropped on this socket.
func (s *SocketStatsV3) GetDrops() uint {
	return uint(s.Drops)
}

// GetQueueFreezes returns the number of queue freezes on this socket.
func (s *SocketStatsV3) GetQueueFreezes() uint {
	return uint(s.Freeze_q_cnt)
}
...
func (h *TPacket) SetFanout(t FanoutType, id uint16) error {
	h.mu.Lock()
	defer h.mu.Unlock()
	// will be converted to int32 by SetsockoptInt which corresponds to C
	// type int on Linux.
	arg := int(t) << 16
	arg |= int(id)
